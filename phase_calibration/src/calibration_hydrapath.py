...
dir_ = '/home/david/Documents/DATA-2/Imaging/14_Aug_26/hydra-pdata-5'
dir__ = '/home/david/Documents/DATA-2/Imaging/14_Aug_26/phase-calibration-5'
path = dir_ 
...
startDate = datetime.date(2026,8,14)
endDate = datetime.date(2026,8,14)
startTime = datetime.time(11,45,0)
endTime = datetime.time(12,15,0)
...
while True:
    ...
    for i in range(0, 28):
        ...
        # arr[i, j] = numpy.mean(spectradataObj.dataOut.data_cspc[i, :, :])
        # arr[i, j] = numpy.mean(spectradataObj.dataOut.data_cspc[i, :, 100:150]) # 250:400 #100:900 # 
        data = spectradataObj.dataOut.data_cspc[i, :, :]

        arr[i, j] = (
            numpy.median(data.real)
            + 1j * numpy.median(data.imag)
        )
...
plt.plot(thx, thy, linewidth=3.0, color='k')
#plt.show()
plt.gcf().clear()
...
for a1 in range(0, 7):
    for a2 in range(a1+1, 8):
        ...
        plt.plot(t, f.imag, 'x') # Experimental point
        ...
        plt.xlabel('Number of hours after {}'.format(start))
